...
func NewCmdApi(f *cmdutil.Factory, runF func(*ApiOptions) error) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: heredoc.Docf(`
			...
			The endpoint argument should be a path of a Bitbucket API 2.0 endpoint.

			Placeholder values %[1]s{workspace}%[1]s, %[1]s{repo}%[1]s, and %[1]s{branch}%[1]s in the endpoint
			argument will get replaced with values from the repository of the current
			directory or the repository specified in the %[1]sBB_REPO%[1]s environment variable.
		...
		Annotations: map[string]string{
			"help:environment": heredoc.Docf(`
				BB_TOKEN: an authentication token (App Password) for %[1]sbitbucket.org%[1]s API requests.
				The token should be in the format %[1]susername:app_password%[1]s.

				BB_HOST: make the request to a Bitbucket host other than %[1]sbitbucket.org%[1]s.
			`, "`"),
		},
