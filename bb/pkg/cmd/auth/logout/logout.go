...
func NewCmdLogout(f *cmdutil.Factory, runF func(*LogoutOptions) error) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: heredoc.Doc(`
			Remove authentication for a Bitbucket account.
		...
		Example: heredoc.Doc(`
			...
			$ bb auth logout --hostname bitbucket.org --user myusername
		`),
	...
	cmd.Flags().StringVarP(&opts.Hostname, "hostname", "h", "", "The hostname of the Bitbucket instance to log out of")
...
func logoutRun(opts *LogoutOptions) error {
	...
	if src, writeable := shared.AuthTokenWriteable(authCfg, hostname); !writeable {
		...
		fmt.Fprint(opts.IO.ErrOut, "To erase credentials stored in Bitbucket CLI, first clear the value from the environment.\n")
