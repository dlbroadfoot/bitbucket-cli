...
func NewCmdSetupGit(f *cmdutil.Factory, runF func(*SetupGitOptions) error) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "setup-git",
		Short: "Setup git with Bitbucket CLI",
		Long: heredoc.Docf(`
			This command configures %[1]sgit%[1]s to use Bitbucket CLI as a credential helper.
			For more information on git credential helpers please reference:
			<https://git-scm.com/docs/gitcredentials>.

			By default, Bitbucket CLI will be set as the credential helper for all authenticated hosts.
		...
		`, "`"),
		Example: heredoc.Doc(`
			# Configure git to use Bitbucket CLI as the credential helper for all authenticated hosts
			$ bb auth setup-git

			# Configure git to use Bitbucket CLI as the credential helper for bitbucket.example.com host
			$ bb auth setup-git --hostname bitbucket.example.com
		`),
