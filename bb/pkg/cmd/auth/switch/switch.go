...
func NewCmdSwitch(f *cmdutil.Factory, runF func(*SwitchOptions) error) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Short: "Switch active Bitbucket account",
		Long: heredoc.Docf(`
			Switch the active account for a Bitbucket host.

			This command changes the authentication configuration that will
			be used when running commands targeting the specified Bitbucket host.
			...
			For a list of authenticated accounts you can run %[1]sbb auth status%[1]s.
		`, "`"),
		Example: heredoc.Doc(`
			...
			$ bb auth switch --hostname bitbucket.org --user myusername
		`),
	...
	cmd.Flags().StringVarP(&opts.Hostname, "hostname", "h", "", "The hostname of the Bitbucket instance to switch account for")
