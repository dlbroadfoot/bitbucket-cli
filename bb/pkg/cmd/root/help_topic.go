...
var HelpTopics = []helpTopic{
	{
		name:  "mintty",
		short: "Information about using bb with MinTTY",
		long: heredoc.Docf(`
			MinTTY is the terminal emulator that comes by default with Git
			for Windows. It has known issues with bb's ability to prompt a
			user for input.

			There are a few workarounds to make bb work with MinTTY:
			...
			- Prefix invocations of bb with %[1]swinpty%[1]s, eg: %[1]swinpty bb auth login%[1]s.
	...
	},
	{
		...
		long: heredoc.Docf(`
			...
			cannot be inferred from the context of a local Git repository.

			%[1]sBB_REPO%[1]s: specify the Bitbucket repository in the %[1]s[HOST/]WORKSPACE/REPO%[1]s format for commands
			that otherwise operate on a local repository.

			%[1]sBB_EDITOR%[1]s, %[1]sGIT_EDITOR%[1]s, %[1]sVISUAL%[1]s, %[1]sEDITOR%[1]s (in order of precedence): the editor tool to use
			for authoring text.

			%[1]sBB_BROWSER%[1]s, %[1]sBROWSER%[1]s (in order of precedence): the web browser to use for opening links.

			%[1]sBB_DEBUG%[1]s: set to a truthy value to enable verbose output on standard error. Set to %[1]sapi%[1]s
			...
			%[1]sBB_PAGER%[1]s, %[1]sPAGER%[1]s (in order of precedence): a terminal paging program to send standard output
			to, e.g. %[1]sless%[1]s.
			...
			%[1]sBB_FORCE_TTY%[1]s: set to any value to force terminal-style output even when the output is
			redirected. When the value is a number, it is interpreted as the number of columns
			available in the viewport. When the value is a percentage, it will be applied against
			the number of columns available in the current viewport.

			%[1]sBB_NO_UPDATE_NOTIFIER%[1]s: set to any value to disable Bitbucket CLI update notifications.
			When any command is executed, bb checks for new versions once every 24 hours.
			If a newer version was found, an upgrade notice is displayed on standard error.

			%[1]sBB_CONFIG_DIR%[1]s: the directory where bb will store configuration files. If not specified,
			the default value will be one of the following paths (in order of precedence):
			  - %[1]s$XDG_CONFIG_HOME/bb%[1]s (if %[1]s$XDG_CONFIG_HOME%[1]s is set),
			  - %[1]s$AppData/Bitbucket CLI%[1]s (on Windows if %[1]s$AppData%[1]s is set), or
			  - %[1]s$HOME/.config/bb%[1]s.

			%[1]sBB_PROMPT_DISABLED%[1]s: set to any value to disable interactive prompting in the terminal.

			%[1]sBB_PATH%[1]s: set the path to the bb executable, useful for when bb can not properly determine
			its own path such as in the cygwin terminal.

			%[1]sBB_MDWIDTH%[1]s: default maximum width for markdown render wrapping.  The max width of lines
			...
			%[1]sBB_ACCESSIBLE_PROMPTER%[1]s (preview): set to a truthy value to enable prompts that are
			more compatible with speech synthesis and braille screen readers.

			%[1]sBB_SPINNER_DISABLED%[1]s: set to a truthy value to replace the spinner animation with
			a textual progress indicator.
		`, "`"),
	},
	{
		name:  "reference",
		short: "A comprehensive reference of all bb commands",
	},
	{
		name:  "formatting",
		short: "Formatting options for JSON data exported from bb",
		long: heredoc.Docf(`
			By default, the result of %[1]sbb%[1]s commands are output in line-based plain text format.
	...
	},
	{
		...
		long: heredoc.Doc(`
			bb follows normal conventions regarding exit codes.
...
func NewCmdHelpTopic(ios *iostreams.IOStreams, ht helpTopic) *cobra.Command {
	cmd := &cobra.Command{
		...
		Annotations: map[string]string{
			...
			"markdown:basename": "bb_help_" + ht.name,
		},
	}
...
func helpTopicUsageFunc(w io.Writer, command *cobra.Command) error {
	fmt.Fprintf(w, "Usage: bb help %s", command.Use)
	return nil
}
