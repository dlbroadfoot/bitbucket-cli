...
func NewCmdList(f *cmdutil.Factory, runF func(*ListOptions) error) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: heredoc.Doc(`
			List pipeline variables (secrets) for a repository.

			By default, lists repository-level variables. Use --environment to list
			variables for a specific deployment environment.

			Note: The actual values of secured variables are not displayed.
		`),
		Example: heredoc.Doc(`
			$ bb secret list
			$ bb secret list --environment production
		`),
		Aliases: []string{"ls"},
		Args:    cobra.NoArgs,
	...
	cmd.Flags().StringVarP(&opts.Environment, "environment", "e", "", "List secrets for a specific deployment environment")
...
func listRun(opts *ListOptions) error {
	...
	if len(variables) == 0 {
		if opts.Environment != "" {
			fmt.Fprintf(opts.IO.Out, "No secrets found for environment %q\n", opts.Environment)
		} else {
			fmt.Fprintln(opts.IO.Out, "No secrets found for this repository")
		}
		return nil
	}
