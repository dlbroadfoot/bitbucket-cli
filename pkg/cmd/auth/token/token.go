...
func NewCmdToken(f *cmdutil.Factory, runF func(*TokenOptions) error) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "token",
		Short: "Print the authentication token bb uses for a hostname and account",
		Long: heredoc.Docf(`
			This command outputs the authentication token for an account on a given Bitbucket host.
	...
	cmd.Flags().StringVarP(&opts.Hostname, "hostname", "h", "", "The hostname of the Bitbucket instance authenticated with")
...
func tokenRun(opts *TokenOptions) error {
	...
	if val == "" {
		errMsg := fmt.Sprintf("no token found for %s", hostname)
		if opts.Username != "" {
