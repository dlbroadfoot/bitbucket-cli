...
func NewCmdDelete(f *cmdutil.Factory, runF func(*DeleteOptions) error) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Short: "Delete a secret from a repository",
		Long: heredoc.Doc(`
			Delete a pipeline secret (secured variable) from a repository.
		...
		Example: heredoc.Doc(`
			...
			$ bb secret delete API_KEY --yes
			$ bb secret delete API_KEY --environment production
		`),
	...
	cmd.Flags().StringVarP(&opts.Environment, "environment", "e", "", "Delete secret from a specific deployment environment")
...
func deleteRun(opts *DeleteOptions) error {
	...
	if !opts.Confirm {
		...
		var msg string
		if opts.Environment != "" {
			msg = fmt.Sprintf("Are you sure you want to delete secret %q from environment %q?",
				opts.SecretName, opts.Environment)
		} else {
			msg = fmt.Sprintf("Are you sure you want to delete secret %q?", opts.SecretName)
		}
	...
	err = deleteSecret(httpClient, repo, opts.SecretName, opts.Environment)
	...
	if opts.IO.IsStdoutTTY() {
		...
		if opts.Environment != "" {
			fmt.Fprintf(opts.IO.Out, "%s Deleted secret %s from environment %s\n",
				cs.SuccessIcon(), cs.Bold(opts.SecretName), cs.Cyan(opts.Environment))
		} else {
			fmt.Fprintf(opts.IO.Out, "%s Deleted secret %s\n",
...
func deleteSecret(client *http.Client, repo bbrepo.Interface, name, environment string) error {
